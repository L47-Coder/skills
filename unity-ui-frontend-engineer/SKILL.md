...
2. 创建治理分支，默认 `unity-ui-frontend-engineer-<日期>`，除非用户指定。
3. 扫描所有预制体的层级、组件，记录在一个文档中。（可开启子代理加速）
4. 盘点所有用户可见前端：静态界面、运行态界面、动态实例、交互、动画、转场、提示和反馈。同样记录。 

5. 在 `work/unity-ui-html-loop/` 记录界面清单、运行态状态矩阵、元素清单、设计系统和组件影响链。

6. 先产出至少 5 个实质不同的全局风格方向概念图。
7. 暂停，展示给用户，等待用户选择方向；用户未选择前禁止继续。
8. 用户选择后，按所选风格在 HTML 中完整设计所有界面；每个 UI Prefab 必须对应一个独立 HTML，运行时生成模板必须有独立 HTML 或状态页。
9. HTML 全界面方案冻结后，才允许把 token、布局、资源和动效导入 Unity。
10. Unity 导入后用 Play Mode、截图/录屏、组件尺寸、Console 和测试验证；发现视觉或布局问题，先回 HTML/token 修正，再同步 Unity。
11. 全局清单中的界面、运行态状态、动态模板和动画转场全部通过后，才允许收尾。

## 风格选择门

2. 必须包含主题语言、排版策略、动效策略、代表界面概念图、适用范围、Unity 回填风险和不选择它的理由。
